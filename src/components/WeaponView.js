...
const mapStateToProps = state => {
    return {
        _weapon: state.weapons.weapon,
        user: state.authUser.user
    }
}

class WeaponView extends Component {
    componentDidMount() {
        document.body.className = 'posts show'
    }

    componentWillMount() {
        this.props.getWeapon(this.props.match.params.id)
    }
    ...
    render() {
        const { text, claps, title, feature_img, author } = this.props._weapon
                            ...
                            <div className="post-stats clearfix">
                                <div className="pull-left">
                                    <div className="like-button-wrapper">
                                        <button onClick={() => this.props.clap(this.props._weapon._id)} className="like-button" data-behavior="trigger-overlay" type="submit">
                    ...
                    <div className="post-metadata-bar" data-page="post-metadata-bar">
                        <div className="flex-container is-inView" data-behavior="animated-metadata">
                            <div className="post-stats flex-container">
                                ...
                                <div>
                                    <a className="response-icon-wrapper" href="https://gunnr1.herokuapp.com/posts/it-s-looking-good#responses">
                                        <i className="fa fa-comment-o"></i>
                                        <span className="response-count" data-behavior="response-count">0</span>
                                    </a>
                                </div>
...
/*function mapStateToProps (state, ownProps) {
    const weapon_id = ownProps.match.params.id
    let weapon = {}
    state.weapons.weapons.forEach((_weapon)=>{
        if(weapon_id == _weapon._id) {
            weapon = _weapon
        }
    })
    return { weapon }
